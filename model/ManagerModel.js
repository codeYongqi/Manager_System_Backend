...
async function insertManager (managerInfo) {
    const rows = await query('insert into manager (name,age,phone,passwd,level)'+ 
    ` values (\'${managerInfo.name}\', ${managerInfo.age},\'${managerInfo.phone}\',\'${managerInfo.passwd}\',${managerInfo.level})`)
    console.log(rows)
    return rows;
}
